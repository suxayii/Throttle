cat > /root/net-tune-pro-v3.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail

# ===================== 基础路径 =====================
ACTIVE_FILE="/etc/sysctl.d/99-net-tune-pro-v3.conf"            # 网络profile托管文件
...
need_root(){
  if [[ "${EUID:-$(id -u)}" -ne 0 ]]; then
    err "请用 root 运行：sudo bash /root/net-tune-pro-v3.sh"
    exit 1
  fi
}
...
common_base() {
cat <<'CONF'
# ===== Generated by net-tune-pro-v3.sh =====
# 通用基础项
...
validate_profile_semantics(){
  ...
  [[ -n "$rmem_max" && -n "$tr" ]] && (( rmem_max < tr )) && { err "rmem_max < tcp_rmem max"; return 1; }
  [[ -n "$wmem_max" && -n "$tw" ]] && (( wmem_max < tw )) && { err "wmem_max < tcp_wmem max"; return 1; }
  return 0
}
...
precheck(){
  ...
  sysctl -a >/dev/null 2>&1 || { err "sysctl读取失败"; return 1; }
...
rollback_last_apply_point(){
  ...
  if sysctl --system >/tmp/net-tune-v3-rollback.log 2>&1; then
    ...
    err "回滚失败，见 /tmp/net-tune-v3-rollback.log"
    return 1
  fi
}
...
save_permanent_baseline(){
  ...
  [[ -f "$marker" ]] && { warn "永久基线已存在: $PERM_DIR"; return 0; }
...
LOCK
  ...
  ok "永久初始备份已创建：$PERM_DIR"

  if cmd_exists chattr; then
    read -rp "是否给 BASELINE_LOCK 加 immutable(chattr +i)？[y/N]: " yn
...
restore_permanent_baseline(){
  ...
  if sysctl --system >/tmp/net-tune-v3-restore-baseline.log 2>&1; then
    ...
    err "回滚失败，见 /tmp/net-tune-v3-restore-baseline.log"
    return 1
  fi
}
...
apply_profile(){
  ...
  render_profile_to_tmp "$p" "$tmp" || { rm -f "$tmp"; err "未知profile"; return 1; }
...
bbr_qdisc_apply(){
  ...
  if [[ "$new_qdisc" == "$qdisc" && "$new_algo" == "$algo" ]]; then
    ok "立即生效：qdisc=$new_qdisc, congestion=$new_algo"
  else
...
update_bootloader(){
  ...
  if cmd_exists update-grub; then
    ...
    warn "未找到 update-grub，系统可能使用其它引导（如U-Boot），请按发行版文档确认。"
    return 0
  fi
}
...
install_specific_joey_kernel(){
  ...
  local api="https://api.github.com/repos/byJoey/Actions-bbr-v3/releases"
  local data arch_filter tags selected idx urls
  ...
  (( idx>=1 && idx<=${#tags[@]} )) || { err "编号越界"; return 1; }
...
check_bbr_v3_status(){
  ...
  if [[ -z "$info_mod" ]]; then
    warn "未检测到 tcp_bbr 模块信息（可能未加载或内核不含该模块）"
  else
    version="$(awk '/^version:/ {print $2}' <<< "$info_mod" | head -n1)"
    [[ -n "$version" ]] && info "tcp_bbr 模块 version: $version" || info "tcp_bbr 模块已存在（未读到 version 字段）"
  fi

  algo="$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || echo unknown)"
  qdisc="$(sysctl -n net.core.default_qdisc 2>/dev/null || echo unknown)"
  echo "当前 congestion_control: $algo"
  echo "当前 default_qdisc: $qdisc"

  if [[ "$algo" == "bbr" ]]; then ok "BBR 已启用"; else warn "当前不是 bbr"; fi
}
...
show_status(){
  ...
  echo -e "${BOLD}当前状态${NC}"
  line
  echo "托管profile文件: $ACTIVE_FILE"
  echo "托管BBR文件    : $BBR_SYSCTL_FILE"
  echo "模块配置文件   : $MODULES_CONF"
  echo "会话profile    : ${CURRENT_PROFILE:-unknown}"
  line
  [[ -f "$ACTIVE_FILE" ]] && { echo "[ACTIVE_FILE]"; cat "$ACTIVE_FILE"; line; }
  [[ -f "$BBR_SYSCTL_FILE" ]] && { echo "[BBR_SYSCTL_FILE]"; cat "$BBR_SYSCTL_FILE"; line; }
...
choose_profile_menu(){
  while true; do
    ...
    read -rp "输入 [0-8]: " c
    case "$c" in
      1)p="balanced";;2)p="aggressive";;3)p="aggressive_safe";;4)p="xray_hy2";;
      5)p="low_1c1g";;6)p="low_2c2g";;7)p="bw_1g";;8)p="bw_10g";;0)return;;
      *) warn "无效输入"; sleep 1; continue;;
    esac
    precheck || { pause; continue; }
    conflict_check
    read -rp "确认应用 $p ? [y/N]: " yn
    [[ "${yn:-N}" =~ ^[Yy]$ ]] && apply_profile "$p" || warn "已取消"
    pause
    return
  done
}
...
bbr_menu(){
  while true; do
    clear
    echo -e "${BOLD}BBR 内核与队列管理菜单（移植版）${NC}"
    line
    echo "1) 安装/更新 BBR v3（最新版）"
    echo "2) 指定版本安装 BBR 内核"
    echo "3) 检查 BBR v3 状态"
    echo "4) 启用 BBR + FQ"
...
main_menu(){
  while true; do
    ...
    echo -e "${BOLD}${BLUE}"
    echo "╔══════════════════════════════════════════════════════════╗"
    echo "║ Net Tune Pro v3（v2.1 + Joey BBR 全功能整合）          ║"
    echo "╚══════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
    echo "1) 预检查阶段"
    echo "2) 冲突检测"
    echo "3) 应用网络 Profile（原子写入）"
...
chmod +x /root/net-tune-pro-v3.sh
bash /root/net-tune-pro-v3.sh
