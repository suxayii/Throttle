...
# Busy Poll (低延迟网络轮询，根据系统核心数动态调优)
net.core.busy_read = $bp
net.core.busy_poll = $bp

# 代理延迟优化：禁用 TCP autocorking（默认攒包再发，代理场景增加延迟抖动）
net.ipv4.tcp_autocorking = 0

# 禁用 TCP 指标缓存（代理目标多变，缓存旧 RTT/cwnd 反而导致慢启动异常）
...
profile_udp_quic(){
    ...
    if [ "$TOTAL_MEM" -lt 2048 ]; then
        ...
        # 原逻辑保持不变（高配用动态计算）
        TOTAL_PAGES=$(( TOTAL_MEM * 256 ))
        UDP_MIN=$(( TOTAL_PAGES / 4 ))
        UDP_PRESSURE=$(( TOTAL_PAGES / 3 ))
        UDP_MAX=$(( TOTAL_PAGES / 2 ))
...
auto_optimize(){
  ...
  # 低配检测：如果检测到 s-ui 服务，优先使用 udp_quic（内部已有低配自适应）
  local has_sui=0
  systemctl is-active s-ui.service >/dev/null 2>&1 && has_sui=1

  if (( total_mem_mb < 2048 || cpu_cores < 2 )); then
    if (( has_sui )); then
      selected_profile="udp_quic"
      reason="低配但检测到 s-ui 服务，使用 UDP 专项（内置低配自适应）"
    else
    ...
    if (( has_sui )); then
      selected_profile="udp_quic"
      reason="中低配但检测到 s-ui 服务，使用 UDP 专项（内置内存自适应）"
    else
...
apply_system_optimizations(){
  ...
  echo -e "${BOLD}[1/10] Virtio 多队列${NC}"
  if cmd_exists ethtool; then
  ...
  # ===== 2. RPS / RFS（软件级多核分发） =====
  echo -e "${BOLD}[2/10] RPS / RFS 多核分发${NC}"
  local rps_mask
  ...
  # ===== 3. 网卡 Ring Buffer 加大 =====
  echo -e "${BOLD}[3/10] Ring Buffer${NC}"
  ...
  if cmd_exists ethtool; then
    ...
    if [[ -n "$max_rx" && -n "$max_tx" ]]; then
      # 优先尝试 4096（HY2 抢包最佳值），超过硬件最大值则用硬件最大值
      local target_rx=$(( max_rx > 4096 ? 4096 : max_rx ))
      local target_tx=$(( max_tx > 4096 ? 4096 : max_tx ))
      if ethtool -G "$nic" rx "$target_rx" tx "$target_tx" 2>/dev/null; then
        ok "Ring Buffer 已设为 RX=$target_rx TX=$target_tx（硬件最大 RX=$max_rx TX=$max_tx）"
      else
        # 回退到硬件最大值
        if ethtool -G "$nic" rx "$max_rx" tx "$max_tx" 2>/dev/null; then
          ok "Ring Buffer 已设为硬件最大值（RX=$max_rx TX=$max_tx）"
        else
  ...
  # ===== 4. 硬件卸载（GSO/GRO/TSO） =====
  echo -e "${BOLD}[4/10] 硬件卸载检查${NC}"
  if cmd_exists ethtool; then
    # 强制开启 GSO, TSO
    ethtool -K "$nic" gso on 2>/dev/null && ok "GSO 已开启" || warn "GSO 开启失败（可能不支持）"
    ethtool -K "$nic" tso on 2>/dev/null && ok "TSO 已开启" || warn "TSO 开启失败（可能不支持）"

    # 强制关闭 GRO / rx-gro-hw / rx-udp-gro-forwarding（不检测状态，直接关！）
    ethtool -K "$nic" gro off 2>/dev/null || true
    ethtool -K "$nic" rx-gro-hw off 2>/dev/null || true
    ethtool -K "$nic" rx-udp-gro-forwarding off 2>/dev/null || true
    ok "GRO / rx-gro-hw / rx-udp-gro-forwarding 已强制关闭"

    # 验证结果
    info "验证 GRO 状态："
    ethtool -k "$nic" 2>/dev/null | awk '/generic-receive-offload|rx-gro-hw|rx-udp-gro-forwarding/' || true
  else
    warn "未安装 ethtool，跳过硬件卸载检查"
  fi

  # ===== 5. 中断合并（减少 CPU 开销） =====
  echo -e "${BOLD}[5/10] 中断合并 (Interrupt Coalescing)${NC}"
  if cmd_exists ethtool; then
  ...
  echo -e "${BOLD}[6/10] 网卡硬中断亲和性 (IRQ Affinity)${NC}"
  local irq_applied=0
...
EOF
  ...
  # ===== 7. XPS 发送端多核分发 =====
  echo -e "${BOLD}[7/10] XPS 发送端多核分发 (Transmit Packet Steering)${NC}"
  local xps_applied=0
  for queue_dir in /sys/class/net/"$nic"/queues/tx-*; do
    [[ -d "$queue_dir" ]] || continue
    if [[ -w "$queue_dir/xps_cpus" ]]; then
      if echo "$rps_mask" > "$queue_dir/xps_cpus" 2>/dev/null; then
        xps_applied=$((xps_applied + 1))
      fi
    fi
  done
  if (( xps_applied > 0 )); then
    ok "XPS 已应用到 $xps_applied 个发送队列（掩码：$rps_mask）"
  else
    info "XPS 设置失败或无发送队列可配置"
  fi

  # ===== 8. Conntrack 连接跟踪优化 =====
  echo -e "${BOLD}[8/10] Conntrack 连接跟踪优化${NC}"
  # 先尝试加载 nf_conntrack 模块
  if cmd_exists modprobe; then
    modprobe nf_conntrack 2>/dev/null || true
  fi
  if [[ -f /proc/sys/net/netfilter/nf_conntrack_max ]]; then
    sysctl -w net.netfilter.nf_conntrack_max=262144 >/dev/null 2>&1 || true
    # 调整 hashsize（通常 = max/4）
    ...
    ok "Conntrack 已优化：max=262144, hashsize=65536, 超时已缩短"
  else
    info "未检测到 nf_conntrack 模块（可能未使用 iptables），跳过"
  fi

  # ===== 9. CPU 性能模式 + 禁用 irqbalance =====
  echo -e "${BOLD}[9/10] CPU 性能模式 / irqbalance${NC}"
  # CPU 调速器切换到 performance
  local gov_applied=0
  for gov_file in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do
    [[ -w "$gov_file" ]] || continue
    if echo "performance" > "$gov_file" 2>/dev/null; then
      gov_applied=$((gov_applied + 1))
    fi
  done
  if (( gov_applied > 0 )); then
    ok "CPU 调速器已切换为 performance（$gov_applied 核）"
  else
    info "CPU 调速器不可写（VPS 通常由宿主机控制，这是正常的）"
  fi
  # 禁用 irqbalance（避免覆盖手动 IRQ 亲和性设置）
  if systemctl is-active irqbalance >/dev/null 2>&1; then
    systemctl stop irqbalance 2>/dev/null || true
    systemctl disable irqbalance 2>/dev/null || true
    ok "irqbalance 已停止并禁用（防止覆盖手动 IRQ 绑定）"
  else
    ok "irqbalance 未运行，无需处理"
  fi

  # ===== 10. HY2 UDP 端口 NOTRACK（跳过 conntrack） =====
  echo -e "${BOLD}[10/10] HY2 UDP 端口 NOTRACK（可选）${NC}"
  if cmd_exists iptables; then
    # 检测 s-ui / Hysteria2 使用的 UDP 端口
    local hy2_port=""
    # 尝试从常见 HY2 配置路径检测端口
    if [[ -f /usr/local/s-ui/db/s-ui.db ]] || systemctl is-active s-ui.service >/dev/null 2>&1; then
      # 如果 s-ui 在运行，检测 UDP 监听端口
      hy2_port="$(ss -ulnp 2>/dev/null | awk '/xray|hysteria|s-ui/{gsub(/.*:/,"",$5); print $5}' | head -1 || true)"
    fi
    if [[ -n "$hy2_port" && "$hy2_port" =~ ^[0-9]+$ ]]; then
      # 清除旧的 NOTRACK 规则（防止重复添加）
      iptables -t raw -D PREROUTING -p udp --dport "$hy2_port" -j NOTRACK 2>/dev/null || true
      iptables -t raw -D OUTPUT -p udp --sport "$hy2_port" -j NOTRACK 2>/dev/null || true
      # 添加 NOTRACK
      if iptables -t raw -A PREROUTING -p udp --dport "$hy2_port" -j NOTRACK 2>/dev/null && \
         iptables -t raw -A OUTPUT -p udp --sport "$hy2_port" -j NOTRACK 2>/dev/null; then
        ok "UDP 端口 $hy2_port 已设置 NOTRACK（跳过连接跟踪，降低 CPU 开销）"
      else
        warn "NOTRACK 设置失败（可能 raw 表不支持）"
      fi
    else
      info "未自动检测到 HY2 UDP 端口"
      read -rp "是否手动输入 HY2/QUIC 监听端口（留空跳过）: " manual_port
      if [[ -n "$manual_port" && "$manual_port" =~ ^[0-9]+$ ]]; then
        iptables -t raw -D PREROUTING -p udp --dport "$manual_port" -j NOTRACK 2>/dev/null || true
        iptables -t raw -D OUTPUT -p udp --sport "$manual_port" -j NOTRACK 2>/dev/null || true
        if iptables -t raw -A PREROUTING -p udp --dport "$manual_port" -j NOTRACK 2>/dev/null && \
           iptables -t raw -A OUTPUT -p udp --sport "$manual_port" -j NOTRACK 2>/dev/null; then
          ok "UDP 端口 $manual_port 已设置 NOTRACK"
          hy2_port="$manual_port"
        else
          warn "NOTRACK 设置失败"
        fi
      else
  ...
  if [[ "${1:-}" == "persist" ]]; then
    # 持久化时传递实际设置的 rx/tx，如果没有检测到则默认 256
    local final_rx="${target_rx:-${max_rx:-256}}"
    local final_tx="${target_tx:-${max_tx:-256}}"
    setup_persistence "$nic" "$cpu_count" "$rps_mask" "$final_rx" "$final_tx"
  else
...
if [[ -d /sys/class/net/\$nic ]]; then
  ...
  # Offload
  ethtool -K \$nic gso on tso on 2>/dev/null
  ethtool -K \$nic gro off rx-gro-hw off rx-udp-gro-forwarding off 2>/dev/null
  # RPS
  ...
  for q in /sys/class/net/\$nic/queues/tx-*; do
    echo $rps_mask > \$q/xps_cpus 2>/dev/null
  done
fi
# Conntrack
modprobe nf_conntrack 2>/dev/null || true
sysctl -w net.netfilter.nf_conntrack_max=262144 2>/dev/null || true
[ -w /sys/module/nf_conntrack/parameters/hashsize ] && echo 65536 > /sys/module/nf_conntrack/parameters/hashsize 2>/dev/null || true
...
restore_system_optimizations(){
  ...
  # 还原 XPS (清零掩码)
  for q in /sys/class/net/"$nic"/queues/tx-*; do
    [[ -w "$q/xps_cpus" ]] && echo 0 > "$q/xps_cpus" 2>/dev/null || true
  done
  ok "已重置 XPS"
...
clear_tc_rules(){
  ...
  info "重新应用当前队列调度器 (${current_qdisc})..."
  tc qdisc add dev "$nic" root "$current_qdisc" 2>/dev/null || true
  
  echo
  ok "已清除所有限速/整形规则，并重置为 ${current_qdisc}。"
...
enable_bbr_fq(){
  ...
  # 清理可能存在的旧配置（同时清理 sysctl.conf 和托管文件，避免跨文件重复键）
  if [[ -f "$bbr_conf" ]]; then
    sed -i '/^net\.core\.default_qdisc/d' "$bbr_conf"
    sed -i '/^net\.ipv4\.tcp_congestion_control/d' "$bbr_conf"
  fi
  if [[ -f "$ACTIVE_FILE" ]]; then
    sed -i '/^net\.core\.default_qdisc/d' "$ACTIVE_FILE"
    sed -i '/^net\.ipv4\.tcp_congestion_control/d' "$ACTIVE_FILE"
